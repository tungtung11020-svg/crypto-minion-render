...
async def backup_database_async() -> Path:
    db_url = get_settings().database_url.strip().lower()
    if not db_url.startswith('sqlite'):
        raise RuntimeError(
            'Backup доступен только для SQLite. Для PostgreSQL используйте панель Neon.'
        )
    return await asyncio.to_thread(backup_database)

def restore_database(backup: Path, confirmation: str) -> None:
    if confirmation != 'ВОССТАНОВИТЬ':
        raise ValueError('Требуется подтверждение ВОССТАНОВИТЬ')
    backup_database()
    src = _path()
    with sqlite3.connect(backup) as s, sqlite3.connect(src) as d:
        s.backup(d)
